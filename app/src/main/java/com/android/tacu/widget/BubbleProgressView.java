...
public class BubbleProgressView extends View {
    ...
    private int inRadiu = UIUtils.dp2px(8);
    private int outRadiu = UIUtils.dp2px(12);

    private float mProgressHeight = UIUtils.dp2px(12);
    private float mBubbleTriangleHeight = UIUtils.dp2px(5);//气泡底部小三角高度
    private float mBubbleRectRound = UIUtils.dp2px(4);//气泡的圆角
    private float mTextSize = UIUtils.dp2px(14);//进度条文字大小
    private float mProgressStrMarginWidth = UIUtils.dp2px(4);//气泡的边距
    private float mProgressStrMarginHight = UIUtils.dp2px(6);//气泡的边距

    private int mProgress = 0;//进度条的进度
    private int mMaxProgress = 100;
    private float depth = mProgress / mMaxProgress;
    ...
    private void init() {
        mColorProgressBg = ContextCompat.getColor(getContext(), R.color.content_bg_color_grey);
        mColorProgress = ContextCompat.getColor(getContext(), R.color.color_default);
        mColorProgressStr = ContextCompat.getColor(getContext(), R.color.text_color);
        mColorIn = ContextCompat.getColor(getContext(), R.color.color_default);
        mColorOut = ContextCompat.getColor(getContext(), R.color.color_otc_unhappy);
    ...
    private void drawProgress(Canvas canvas) {
        ...
        float stop = mPathMeasure.getLength() * depth;//计算进度条的进度
    ...
    private void drawBubble(Canvas canvas) {
        ...
        int width = (int) (rect.width() + mProgressStrMarginWidth);//计算字符串宽度(加上设置的边距)
        int height = (int) (rect.height() + mProgressStrMarginHight);//计算字符串高度(加上设置的边距)
        mPathBubble.reset();
    ...
    public void setProgress(int progress, int max) {
        mProgress = progress;
        mMaxProgress = max;
        depth = (float) mProgress / (float) mMaxProgress;
        invalidate();
    }
}
