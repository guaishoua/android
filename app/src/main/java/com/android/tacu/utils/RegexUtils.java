...
public class RegexUtils {
     ...
     * @param editText
     */
    public static void setEditTextPwd(EditText editText) {
        InputFilter filter = new InputFilter() {
            @Override
            public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
                String speChat = "[`~!@#$%^&*()+=|{}':;',\\[\\]<>/?~！@#￥%……&*（） ”“ ——+|{}【】‘；：”“’。，、？]";
    ...
    public static boolean checkName(String uname) {
        if (uname.length() < 2) {
            ShowToast.error("用户名不能小于两位");
            return Boolean.FALSE;
        }
        //用户名不能是全数字
        if (uname.matches("[0-9]+") || uname.length() > 30 || uname.length() < 2) {
            ShowToast.error("用户名不能是全数字");
            return false;
        }
        //包含非法字符
        if (uname.contains("admin") || uname.contains("null")) {
            ShowToast.error("包含非法字符");
            return false;
        }
     ...
     * @return
     */
    public static boolean findPwd(String pwd) {
        if (pwd.length() < 6) {
            ShowToast.error("密码不能低于6位");
            return Boolean.FALSE;
        }
        if (pwd.length() > 18) {
            ShowToast.error("密码不能超过18位");
            return Boolean.FALSE;
        }
        //用户名不能是全数字
        if (pwd.matches("[0-9]+")) {
            ShowToast.error("密码不能为纯数字");
            return false;
        }
     ...
     * @return 验证成功返回true，验证失败返回false
     */
    public static boolean checkEmail(String email, int county) {
        String regex = "^([\\w-\\.]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([\\w-]+\\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$";
        return Pattern.matches(regex, email);
    }

    public static boolean checkAccount(boolean isEmail, Context context, String email, int county) {
        if (isEmail) {
            if (TextUtils.isEmpty(email)) {
                ShowToast.error(context.getResources().getString(R.string.email_null));
                return false;
            }
            if (!RegexUtils.checkEmail(email, county) || email.length() < 6) {
                ShowToast.error(context.getResources().getString(R.string.invalid_email_address));
                return false;
            }
        } else {
            if (TextUtils.isEmpty(email)) {
                ShowToast.error(context.getResources().getString(R.string.phone_null));
                return false;
            }
            //国内、国外手机号判断
            if (county != 86 && email.length() < 3 || email.length() > 20) {
                ShowToast.error(context.getResources().getString(R.string.phone_invalia));
                return false;
            } else if (county == 86 && (email.length() != 11 || !TextUtils.equals(email.substring(0, 1), "1"))) {
                ShowToast.error(context.getResources().getString(R.string.phone_invalia));
                return false;
            }
        }
