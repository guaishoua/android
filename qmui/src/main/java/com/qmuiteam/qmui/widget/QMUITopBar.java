...
public class QMUITopBar extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTopBarSeparatorColor = array.getColor(R.styleable.QMUITopBar_qmui_topbar_separator_color, ContextCompat.getColor(context, R.color.qmui_config_color_separator));
    ...
    public TextView setTitle(String title, int resId) {
        return setTitle(title, resId, 26, 26, 0);
    }

    /**
     * 添加 TopBar 的标题旁边的小图标
     */
    public TextView setTitle(String title, int resId, int width, int height, int drawpadding) {
        TextView titleView = getTitleView();
        titleView.setText(title);
        Drawable drawable = null;
        if (resId != 0) {
            drawable = ContextCompat.getDrawable(getContext(), resId);
            drawable.setBounds(0, 0, QMUIDisplayHelper.dpToPx(width), QMUIDisplayHelper.dpToPx(height));
        }
        titleView.setCompoundDrawables(null, null, drawable, null);
        titleView.setCompoundDrawablePadding(QMUIDisplayHelper.dpToPx(drawpadding));
        if (QMUILangHelper.isNullOrEmpty(title)) {
            titleView.setVisibility(GONE);
        } else {
            titleView.setVisibility(VISIBLE);
     ...
     */
    public LayoutParams generateTopBarLeftImageButtonLayoutParams() {
        return generateTopBarLeftImageButtonLayoutParams(mTopBarImageBtnWidth, mTopBarImageBtnWidth);
    }

    public LayoutParams generateTopBarLeftImageButtonLayoutParams(int width, int height) {
        LayoutParams lp = new LayoutParams(width, height);
        lp.topMargin = Math.max(0, (getTopBarHeight() - height) / 2);
        lp.rightMargin = mTopBarTextBtnPaddingHor;
        return lp;
    }
     ...
     * 生成一个 LayoutParams，当把 Button addView 到 TopBar 时，使用这个 LayouyParams
     */
    public LayoutParams generateTopBarRightImageButtonLayoutParams() {
        return generateTopBarRightImageButtonLayoutParams(mTopBarImageBtnWidth, mTopBarImageBtnWidth);
    }

    public LayoutParams generateTopBarRightImageButtonLayoutParams(int width, int height) {
        LayoutParams lp = new LayoutParams(width, height);
        lp.topMargin = Math.max(0, (getTopBarHeight() - height) / 2);
        lp.leftMargin = mTopBarTextBtnPaddingHor;
        return lp;
    }
    ...
    public QMUIAlphaImageButton addLeftImageButton(int drawableResId, int viewId) {
        QMUIAlphaImageButton leftButton = generateTopBarLeftImageButton(drawableResId);
        this.addLeftView(leftButton, viewId, generateTopBarLeftImageButtonLayoutParams());
        return leftButton;
    }

    public QMUIAlphaImageButton addLeftImageButton(int drawableResId, int viewId, int width, int height) {
        QMUIAlphaImageButton leftButton = generateTopBarLeftImageButton(drawableResId);
        this.addLeftView(leftButton, viewId, generateTopBarLeftImageButtonLayoutParams(QMUIDisplayHelper.dpToPx(width), QMUIDisplayHelper.dpToPx(height)));
        return leftButton;
    }
    ...
    public QMUIAlphaImageButton addRightImageButton(int drawableResId, int viewId) {
        QMUIAlphaImageButton rightButton = generateTopBarRightImageButton(drawableResId);
        this.addRightView(rightButton, viewId, generateTopBarRightImageButtonLayoutParams());
        return rightButton;
    }

    public QMUIAlphaImageButton addRightImageButton(int drawableResId, int viewId, int width, int height) {
        QMUIAlphaImageButton rightButton = generateTopBarRightImageButton(drawableResId);
        this.addRightView(rightButton, viewId, generateTopBarRightImageButtonLayoutParams(QMUIDisplayHelper.dpToPx(width), QMUIDisplayHelper.dpToPx(height)));
        return rightButton;
    }

    /**
     * 生成一个左边LayoutParams，当把 Button addView 到 TopBar 时，使用这个 LayouyParams
     */
    public LayoutParams generateTopBarLeftTextButtonLayoutParams() {
    ...
    /**
     * 生成一个右边LayoutParams，当把 Button addView 到 TopBar 时，使用这个 LayouyParams
     */
    public LayoutParams generateTopBarRightTextButtonLayoutParams() {
        LayoutParams lp = new LayoutParams(LayoutParams.WRAP_CONTENT, mTopBarImageBtnHeight);
        lp.topMargin = Math.max(0, (getTopBarHeight() - mTopBarImageBtnHeight) / 2);
        lp.leftMargin = mTopBarTextBtnPaddingHor;
        return lp;
    ...
    public Button addLeftTextButton(String buttonText, int viewId) {
        Button button = generateTopBarTextLeftButton(buttonText);
        this.addLeftView(button, viewId, generateTopBarLeftTextButtonLayoutParams());
        return button;
    }
    ...
    public Button addRightTextButton(String buttonText, int viewId) {
        Button button = generateTopBarTextRightButton(buttonText);
        this.addRightView(button, viewId, generateTopBarRightTextButtonLayoutParams());
        return button;
    }
     ...
     * @return 返回生成的按钮
     */
    private Button generateTopBarTextLeftButton(String text) {
        ...
        button.setPadding(0, 0, mTopBarTextBtnPaddingHor, 0);
    ...
    private Button generateTopBarTextRightButton(String text) {
        Button button = new Button(getContext());
        button.setBackgroundResource(0);
        button.setMinWidth(0);
        button.setMinHeight(0);
        button.setMinimumWidth(0);
        button.setMinimumHeight(0);
        button.setPadding(mTopBarTextBtnPaddingHor, 0, 0, 0);
        button.setTextColor(mTopBarTextBtnTextColor);
        button.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTopBarTextBtnTextSize);
        button.setGravity(Gravity.CENTER);
        button.setText(text);
    ...
    /**
     * 生成一个左边图片按钮，配合 {{@link #generateTopBarLeftImageButtonLayoutParams()} 使用
     *
     * @param imageResourceId 图片的 resId
     */
    private QMUIAlphaImageButton generateTopBarLeftImageButton(int imageResourceId) {
    ...
    /**
     * 生成一个左边图片按钮，配合 {{@link #generateTopBarRightImageButtonLayoutParams()} 使用
     *
     * @param imageResourceId 图片的 resId
     */
    private QMUIAlphaImageButton generateTopBarRightImageButton(int imageResourceId) {
        QMUIAlphaImageButton backButton = new QMUIAlphaImageButton(getContext());
        backButton.setBackgroundColor(Color.TRANSPARENT);
        backButton.setImageResource(imageResourceId);
        backButton.setScaleType(ImageView.ScaleType.CENTER_CROP);
    ...
    public QMUIAlphaImageButton addLeftBackImageButton() {
        return addLeftImageButton(mLeftBackDrawableRes, R.id.qmui_topbar_item_left_back, 22, 22);
    }
    ...
    public void setBackgroundDividerEnabled(boolean enabled) {
        if (enabled) {
            if (mTopBarBgWithSeparatorDrawableCache == null) {
                mTopBarBgWithSeparatorDrawableCache = QMUIDrawableHelper.createItemSeparatorBg(mTopBarSeparatorColor, mTopBarBgColor, mTopBarSeparatorHeight, false);
            }
