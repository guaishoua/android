...
/**
 * 滚动的字幕
 * Created by jiazhen on 2018/9/7.
 */
public class QMUIScrollTextView extends ViewFlipper {
    ...
    public QMUIScrollTextView(Context context) {
        this(context, null);
    }

    public QMUIScrollTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.QMUIScrollTextView, defStyleAttr, 0);

        interval = typedArray.getInteger(R.styleable.QMUIScrollTextView_qmui_scrolltext_interval, interval);
        hasSetAnimDuration = typedArray.hasValue(R.styleable.QMUIScrollTextView_qmui_scrolltext_animduration);
        animDuration = typedArray.getInteger(R.styleable.QMUIScrollTextView_qmui_scrolltext_animduration, animDuration);
        singleLine = typedArray.getBoolean(R.styleable.QMUIScrollTextView_qmui_scrolltext_singleline, false);
        if (typedArray.hasValue(R.styleable.QMUIScrollTextView_qmui_scrolltext_textsize)) {
            textSize = (int) typedArray.getDimension(R.styleable.QMUIScrollTextView_qmui_scrolltext_textsize, textSize);
            textSize = QMUIDisplayHelper.px2dp(context, textSize);
        }
        textColor = typedArray.getColor(R.styleable.QMUIScrollTextView_qmui_scrolltext_textcolor, textColor);

        int gravityType = typedArray.getInt(R.styleable.QMUIScrollTextView_qmui_scrolltext_gravity, GRAVITY_LEFT);
        switch (gravityType) {
            case GRAVITY_LEFT:
        ...
        hasSetDirection = typedArray.hasValue(R.styleable.QMUIScrollTextView_qmui_scrolltext_direction);
        direction = typedArray.getInt(R.styleable.QMUIScrollTextView_qmui_scrolltext_direction, direction);
        if (hasSetDirection) {
            switch (direction) {
    ...
    private void startWithFixedWidth(String notice, @AnimRes int inAnimResId, @AnimRes int outAnimResID) {
        ...
        int width = QMUIDisplayHelper.pxToDp(getWidth());
        if (width == 0) {
