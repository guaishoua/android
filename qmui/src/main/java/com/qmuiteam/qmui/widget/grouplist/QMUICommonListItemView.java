...
public class QMUICommonListItemView extends RelativeLayout {
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mLineView = findViewById(R.id.group_list_item_line);
    ...
    public void setTextContainerPadding(int left, int top, int right, int bottom) {
        mTextContainer.setPadding(left, top, right, bottom);
    }
    ...
    /**
     * 使用这个方法 必须设置QMUIGroupListView 的mSeparatorStyle 为NONE
     * QMUIGroupListView的第一个QMUICommonListItemView不需要设置这个方法 因为这条线在上面
     *
     * @param colorInt
     * @param height
     * @param marginLeft
     * @param marginRight
     */
    public void setLineView(@ColorInt int colorInt, int height, int marginLeft, int marginRight) {
        mLineView.setVisibility(VISIBLE);
        mLineView.setBackgroundColor(colorInt);
        LayoutParams param = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, QMUIDisplayHelper.dpToPx(height));
        param.setMargins(QMUIDisplayHelper.dpToPx(marginLeft), 0, QMUIDisplayHelper.dpToPx(marginRight), 0);
        mLineView.setLayoutParams(param);
    }

    public void setLineView(@ColorInt int colorInt, int marginLeft, int marginRight) {
        setLineView(colorInt, 1, marginLeft, marginRight);
    }

    public void setLineView(@ColorInt int colorInt) {
        setLineView(colorInt, 1, 0, 0);
    }
