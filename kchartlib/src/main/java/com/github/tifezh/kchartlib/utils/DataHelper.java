...
public class DataHelper {
    ...
    static void calculateKDJ(List<KLineEntity> datas) {
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            float max9 = datas.get(startIndex).getHighPrice();
            float min9 = datas.get(startIndex).getLowPrice();
     ...
     * WR2一般是6天买卖强弱指标
     */
    static void calculateWR(List<KLineEntity> datas) {
        float wr1 = 0.0f;
        float wr2 = 0.0f;
        for (int i = 0; i < datas.size(); i++) {
            KLineEntity point = datas.get(i);
            ...
            float max10 = datas.get(startIndex10).getHighPrice();
            float min10 = datas.get(startIndex10).getLowPrice();
            for (int index = startIndex10; index <= i; index++) {
                max10 = Math.max(max10, datas.get(index).getHighPrice());
                min10 = Math.min(min10, datas.get(index).getLowPrice());
            }
            if (max10 != min10) {
            ...
            float max6 = datas.get(startIndex6).getHighPrice();
            float min6 = datas.get(startIndex6).getLowPrice();
            for (int index = startIndex6; index <= i; index++) {
                max6 = Math.max(max6, datas.get(index).getHighPrice());
                min6 = Math.min(min6, datas.get(index).getLowPrice());
            }
            if (max6 != min6) {
