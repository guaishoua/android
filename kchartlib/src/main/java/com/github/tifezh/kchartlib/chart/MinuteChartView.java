...
public class MinuteChartView extends View implements GestureDetector.OnGestureListener {
    ...
    private void init() {
        ...
        mVolumePaintGreen.setColor(ContextCompat.getColor(getContext(), R.color.chart_blue));
    ...
    private void drawValue(Canvas canvas, int index) {
        ...
        if (index >= 0 && index < mPoints.size()) {
            ...
            String text = getResources().getString(R.string.vol_price) + ":" + floatToString(point.getPrice()) + " ";
            float x = 0;
            canvas.drawText(text, x, y, mPricePaint);
            x += mPricePaint.measureText(text);
            text = getResources().getString(R.string.ave_price) + ":" + floatToString(point.getAvgPrice()) + " ";
